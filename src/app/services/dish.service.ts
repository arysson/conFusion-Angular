...
export class DishService {
  ...
  getDishes(): Observable<Dish[]> {
    return this.afs.collection<Dish>('dishes').snapshotChanges().pipe(map(actions => actions.map(action => {
  ...
  getDish(id: string): Observable<Dish> {
    return this.afs.doc<Dish>('dishes/' + id).snapshotChanges().pipe(map(action => {
  ...
  getFeaturedDish(): Observable<Dish> {
    return this.afs.collection<Dish>('dishes', ref => ref.where('featured', '==', true)).snapshotChanges().pipe(map(actions => actions.map(action => {
      const data = action.payload.doc.data() as Dish;
      const _id = action.payload.doc.id;
      return { _id, ... data };
    })[0]));
  ...
  postComment(dishId: string, comment: any): Promise<any> {
    if (this.currentUser) {
      return this.afs.collection('dishes').doc(dishId).collection('comments').add({
        author: {
